...
태그는 **표시 레이블**과 **URL 슬러그**가 분리되어 동작합니다. 포스트에는 사람이 읽기 좋은 형태로 작성하면 되고, URL 변환은 테마가 자동으로 처리합니다.

```yaml
tags:
  - Apple TV+          # 표시: #Apple TV+  →  URL: /tags/apple-tv-plus/
  - Philip K. Dick     # 표시: #Philip K. Dick  →  URL: /tags/philip-k-dick/
  - MCU                # 표시: #MCU  →  URL: /tags/mcu/
  - 스파이더맨          # 표시: #스파이더맨  →  URL: /tags/스파이더맨/
  - 행복을 찾아서       # 표시: #행복을 찾아서  →  URL: /tags/행복을-찾아서/
```
...
| 입력 태그 | 생성 URL |
|---|---|
| `Apple TV+` | `/tags/apple-tv-plus/` |
| `Philip K. Dick` | `/tags/philip-k-dick/` |
| `MCU` | `/tags/mcu/` |
| `Black Panther` | `/tags/black-panther/` |
| `스파이더맨` | `/tags/스파이더맨/` |
| `행복을 찾아서` | `/tags/행복을-찾아서/` |

- 영문: 소문자 변환, 공백 → 하이픈, `+` → `-plus`, `.` → `-`, 기타 특수문자 제거
- 한글/CJK: 공백 → 하이픈, 특수문자 제거 (한글 자체는 유지)

### 권장 작성 방식

**영문 태그**는 가능하면 공백 없이 작성하는 것이 URL이 깔끔합니다.
...
**같은 개념의 태그는 통일**해서 사용하세요. `MCU`와 `mcu`는 같은 URL로 변환되므로 중복 태그 페이지가 생기지 않지만, 태그 클라우드에서 두 개로 표시됩니다.
...
3~5개가 적당합니다. 너무 많으면 태그의 의미가 희석됩니다.
